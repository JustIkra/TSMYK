...
def test_parse_hysteria2_uri_valid() -> None:
    """Test parsing a valid Hysteria2 URI."""
    uri = "hysteria2://mypassword@example.com:443/?sni=example.net"
    result = vpn._parse_hysteria2_uri(uri)

    assert result["password"] == "mypassword"
    assert result["server"] == "example.com"
    assert result["port"] == "443"
    assert result["sni"] == "example.net"


def test_parse_hysteria2_uri_invalid_scheme() -> None:
    """Test that invalid URI scheme raises error."""
    uri = "https://example.com:443/"

    with pytest.raises(vpn.VpnBootstrapError, match="must start with 'hysteria2://'"):
        vpn._parse_hysteria2_uri(uri)


def test_parse_hysteria2_uri_missing_password() -> None:
    """Test that missing password raises error."""
    uri = "hysteria2://example.com:443/"

    with pytest.raises(vpn.VpnBootstrapError, match="missing password"):
        vpn._parse_hysteria2_uri(uri)


def test_parse_hysteria2_uri_missing_port() -> None:
    """Test that missing port raises error."""
    uri = "hysteria2://mypassword@example.com/"

    with pytest.raises(vpn.VpnBootstrapError, match="missing port"):
        vpn._parse_hysteria2_uri(uri)


def test_bootstrap_from_env_hysteria2_requires_uri() -> None:
    """Test that Hysteria2 bootstrap requires HYSTERIA2_URI."""
    with pytest.raises(vpn.VpnBootstrapError, match="HYSTERIA2_URI is required"):
        vpn.bootstrap_from_env(
            {
                "VPN_ENABLED": "1",
                "VPN_TYPE": "hysteria2",
...
def test_bootstrap_from_env_hysteria2_calls_ensure(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test that Hysteria2 bootstrap calls ensure_hysteria2_up."""
    captured: dict[str, object] = {}

    def fake_ensure(uri: str, socks5_port: int, http_port: int, config_path: str, **_kwargs):
        captured["uri"] = uri
        captured["socks5_port"] = socks5_port
        captured["http_port"] = http_port
        captured["config_path"] = config_path

    monkeypatch.setattr(vpn, "ensure_hysteria2_up", fake_ensure)

    executed = vpn.bootstrap_from_env(
        {
            "VPN_ENABLED": "1",
            "VPN_TYPE": "hysteria2",
            "HYSTERIA2_URI": "hysteria2://pass@example.com:443/",
            "HYSTERIA2_SOCKS5_PORT": "1080",
            "HYSTERIA2_HTTP_PORT": "8080",
            "HYSTERIA2_CONFIG_PATH": "/tmp/hysteria2.yaml",
        }
    )

    assert executed is True
    assert captured["uri"] == "hysteria2://pass@example.com:443/"
    assert captured["socks5_port"] == 1080
    assert captured["http_port"] == 8080
    assert captured["config_path"] == "/tmp/hysteria2.yaml"


def test_ensure_hysteria2_up_generates_config(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Test that ensure_hysteria2_up generates proper YAML config file."""
    config_path = tmp_path / "hysteria2.yaml"

    monkeypatch.setattr(vpn.shutil, "which", lambda name: "/usr/bin/hysteria")

    # Mock socket to simulate port already listening
    class FakeSocket:
        ...
        def connect(self, addr):
            # Simulate successful connection (port already open)
            pass

        def close(self):
            pass

    monkeypatch.setattr(vpn.socket, "socket", FakeSocket)

    uri = "hysteria2://mypassword@example.com:8443/?sni=example.net"
    vpn.ensure_hysteria2_up(uri, 1080, 8080, str(config_path))
    ...
    assert "server: example.com:8443" in content
    assert "auth: mypassword" in content
    assert "sni: example.net" in content
    assert "listen: 127.0.0.1:1080" in content
    assert "listen: 127.0.0.1:8080" in content


def test_collect_hysteria2_status_process_not_running(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test that collect_hysteria2_status detects when Hysteria process is not running."""
    from app.services import vpn_health

    # Mock pgrep to return empty (process not found)
    def fake_run(cmd, capture_output=True, text=True, timeout=None):
        if "pgrep" in cmd:
            return FakeResult(returncode=1, stdout="")
        if "ps" in cmd:
            return FakeResult(returncode=0, stdout="USER PID CMD\nroot 1 bash\n")
        raise AssertionError(f"Unexpected command: {cmd}")

    monkeypatch.setattr(vpn_health.subprocess, "run", fake_run)

    status = vpn_health.collect_hysteria2_status(1080, 8080, "example.com:443")

    assert status.is_running is False
    assert status.socks5_accessible is False
    assert status.http_accessible is False
    assert status.error == "Hysteria process not running"
    assert status.socks5_port == 1080
    assert status.http_port == 8080
    assert status.server == "example.com:443"
