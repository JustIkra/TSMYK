...
@pytest.mark.asyncio
async def test_get_all_users__admin__returns_all_users(
    test_env, client: AsyncClient, db_session: AsyncSession
):
    ...
    await create_user(db_session, "user1@example.com", "password123")
    await create_user(db_session, "user2@example.com", "password123")
    ...
    # Get all users
    response = await client.get("/api/admin/users", cookies=dict(login_response.cookies))
...
@pytest.mark.asyncio
async def test_make_admin__admin__promotes_user(
    test_env, client: AsyncClient, db_session: AsyncSession
):
    """Test admin can grant ADMIN role to a regular user."""
    # Create admin
    admin = await create_user(db_session, "admin@example.com", "password123", role="ADMIN")
    admin.status = "ACTIVE"
    await db_session.commit()
    ...
    response = await client.post(
        f"/api/admin/make-admin/{user_id}", cookies=dict(login_response.cookies)
    )
...
@pytest.mark.asyncio
async def test_delete_user__admin__deletes_user(
    test_env, client: AsyncClient, db_session: AsyncSession
):
    ...
    response = await client.delete(
        f"/api/admin/users/{user_id}", cookies=dict(login_response.cookies)
    )
...
@pytest.mark.asyncio
async def test_delete_user__admin_cannot_delete_self(
    test_env, client: AsyncClient, db_session: AsyncSession
):
    ...
    # Try to delete self
    response = await client.delete(
        f"/api/admin/users/{admin.id}", cookies=dict(login_response.cookies)
    )
