...
4. ДВУХПОЛЮСНЫЕ ШКАЛЫ (ОЧЕНЬ ВАЖНО!):
   - На диаграммах слева и справа от столбца указаны ДВА названия (например: "ВНЕШНЯЯ МОТИВАЦИЯ" слева, "ВНУТРЕННЯЯ МОТИВАЦИЯ" справа)
   - Это РАЗНЫЕ метрики! Извлекай ОБЕ с ОДИНАКОВЫМ значением!
   - Пример: столбец показывает 5 → извлеки: {"label": "ВНЕШНЯЯ МОТИВАЦИЯ", "value": "5"} И {"label": "ВНУТРЕННЯЯ МОТИВАЦИЯ", "value": "5"}
   - Другие пары: ЗАМКНУТОСТЬ/ОБЩИТЕЛЬНОСТЬ, ПАССИВНОСТЬ/АКТИВНОСТЬ, ИМПУЛЬСИВНОСТЬ/ОРГАНИЗОВАННОСТЬ и т.д.
