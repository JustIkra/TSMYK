...
async def seed_metric_defs():
    ...
    # Get all unique metric codes from unified header_map
    all_mappings = mapping_service.get_all_mappings()
    all_codes = set(all_mappings.values())

    print(f"Total unique metric codes: {len(all_codes)}")
    print(f"Total labels (including synonyms): {len(all_mappings)}")
